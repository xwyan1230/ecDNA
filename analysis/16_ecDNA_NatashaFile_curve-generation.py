...
for p in prefix:
    data = pd.read_csv('%sauto_correlation_nuclear_mean_%s.txt' % (master_folder, p), na_values=['.'], sep='\t')
    data['g_FISH'] = [dat.str_to_float(data['g_FISH'][i]) for i in range(len(data))]

    number_nuclear = len(data)
    n_nuclear.append(number_nuclear)

    FISH_mean_curve, FISH_ci_lower, FISH_ci_higher = dat.mean_list(data['g_FISH'].tolist())
