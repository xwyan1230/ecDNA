    ...
    for z in range(total_z):
        ...
        for i in range(len(nuclear_props)):
            ...
            if nuclear_props[i].area * 1.0 / nuclear_props[i].convex_area > 0.9:
                ...
                FISH_seg, _ = seg.find_organelle(FISH, 'na', extreme_val=500, bg_val=seg.get_bg_int([FISH])[0],
                                                 min_size=0, max_size=10000)

                bg_seg = nuclear_seg.copy()
                bg_seg[FISH_seg == 1] = 0
                bg_props = regionprops(label(bg_seg), FISH)
                bg, max_area = seg.get_mean_int_of_max_area(bg_props)

                if max_area > 2000:

                    bg_img = np.ones_like(FISH) * int(bg)
                    FISH_correct = img.image_deduction(FISH, bg_img)
                    ...
                    _, r, g_FISH, dg_FISH = mat.auto_correlation(FISH_correct, nuclear_seg, rmax)
                    _, r, g_nuclear, dg_nuclear = mat.auto_correlation(nuclear, nuclear_seg, rmax)

                    data.loc[len(data.index)] = \
                        [fov, z, i + 1, original_centroid, nuclear_area, nuclear_circ, FISH_mean_int, g_FISH, dg_FISH,
                         g_nuclear, dg_nuclear]
            else:
