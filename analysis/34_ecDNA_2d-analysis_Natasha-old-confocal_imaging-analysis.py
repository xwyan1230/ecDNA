...
# OPEN Z FILE
data_z = pd.read_csv('%s3hrJQ1_3hr1uMtriptolide_WO_intensity.txt' % master_folder, na_values=['.'], sep='\t')
    ...
    for i in range(len(data_z_fov_max)):
        ...
        if (len(data_temp) == len(set(data_temp['z'].tolist()))) & (len(data_temp) >= 3):
            ...
            for j in range(3):
                ...
                data.loc[len(data.index)] = [fov, i, z_nuclear_temp, label_nuclear_temp, original_centroid_nuclear_temp,
...
for x in range(int(len(data)/3)):
    ...
    data_mean.loc[len(data_mean.index)] = [data['FOV'][3*x], data['nuclear'][3*x], data['centroid_nuclear'][3*x],
                                           data['area_nuclear'][3*x], data['major_axis_nuclear'][3*x],
                                           data['minor_axis_nuclear'][3*x], data['major_and_minor_axis_ratio_nuclear'][3*x],
